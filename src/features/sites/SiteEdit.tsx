...
const SiteEdit = () => {
    ...
    const { id } = useParams();
    const { selectedSite, errors, notification } = useAppSelector((state) => state.sites)

    useEffect(() => {
        dispatch(fetchSiteById(Number(id)));
        dispatch(clearNotif());
        dispatch(clearErrors());
    }, [])

    useEffect(() => {
        setItem({...item, id: selectedSite.id, name: selectedSite.name})
        console.log(selectedSite);
    }, [selectedSite])

    useEffect(()=>{
        if(notification){
            dispatch(addNotif({type: notification.type, msg: notification.msg, feature: 'sites'}));
