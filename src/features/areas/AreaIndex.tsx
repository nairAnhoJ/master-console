...
const AreaIndex = () => {
    ...
    const { areas, loading, notification } = useAppSelector((state) => state.areas);
    const { feature } = useAppSelector((state) => state.notification);

    useEffect(() => {
        dispatch(fetchAreas(search));
    }, [])
    ...
    const handleConfirm = () => {
        dispatch(deleteArea(selectedId))
    }
        ...
        <>  
            ...
            <div className="w-screen h-screen pl-[264px] p-6 bg-[#232323] text-gray-300">
                ...
                <div className="w-full h-10 mt-6 flex justify-between">
                    <div className="w-44">
                        <Link to={'/areas/add'} className="block p-2 font-bold text-white bg-blue-700 over:bg-blue-800 rounded cursor-pointer shadow text-center">New Area</Link>
                    </div>
                    ...
                    <div className="w-[350px] h-full overflow-auto">
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                ...
                                { areas?.map((area) => (
                                    <tr className="cursor-pointer hover:bg-[#303030] focus:bg-[#303030]" key={area.id}>
                                        <td className="text-left px-6">{area.name}</td>
                                        <td className="text-center whitespace-nowrap">
                                            <Link to={`/areas/edit/${area.id}`} className="text-blue-500 font-semibold cursor-pointer">EDIT</Link>
                                            <span className="mx-1 cursor-default">|</span> 
                                            <button onClick={()=>handleDeleteButton(area.id)} className="text-red-500 font-semibold cursor-pointer">DELETE</button>
                                        </td>
                                    </tr>
                                )) }
                            </tbody>
