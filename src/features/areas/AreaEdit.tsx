...
const AreaEdit = () => {
    ...
    const { id } = useParams();
    const { selectedArea, errors, notification } = useAppSelector((state) => state.areas)

    useEffect(() => {
        dispatch(fetchAreaById(Number(id)));
        dispatch(clearNotif());
        dispatch(clearErrors());
    }, [])

    useEffect(() => {
        setItem({...item, id: selectedArea.id, name: selectedArea.name})
        console.log(selectedArea);
    }, [selectedArea])

    useEffect(()=>{
        if(notification){
            dispatch(addNotif({type: notification.type, msg: notification.msg, feature: 'areas'}));
            ...
            <div className="fixed w-screen h-screen bg-[#232323] pl-[264px] p-6 text-gray-300 overflow-y-auto">
                <div className="w-96">
                    ...
                    <div className="flex items-center gap-x-6">
                        <Link to={"/areas"} className="bg-[#282828] hover:bg-[#252525] rounded w-32 h-10 border border-[#363636] shadow shadow-[#181818] relative cursor-pointer">
                        ...
                        <h1 className="text-xl font-bold">Edit Area</h1>
                    </div>
